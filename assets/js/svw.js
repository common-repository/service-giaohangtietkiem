jQuery(document).ready(function(){
	...
	if ( jQuery('.svw-ghtk-select-city').length > 0 ) {
		jQuery('.svw-ghtk-select-city').on( 'change', function(){
			jQuery.ajax({
				type: 'POST',
			  	url: svw.ajax.url,
			  	data: {
				...
				jQuery('.svw-ghtk-select-district').html(result);
			});
		});
	}

	if ( jQuery('.svw-ghtk-select-district').length > 0 ) {
		jQuery('.svw-ghtk-select-district').on('change', function(){
			jQuery.ajax({
				type: 'POST',
			  	url: svw.ajax.url,
			  	data: {
				...
				jQuery('.svw-ghtk-select-ward').html(result);
			});
		});
	}
